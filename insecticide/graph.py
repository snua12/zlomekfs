""" Module with code handling test dependency graphs.
    Load them from test class, create graph and generate 
...
class DependencyGraph(object):
    ...
    def equals(self,  graph):
        """ Compares this graph with given and returns 
            if they contains equal information.
        ...
        """
        return self.graph == graph.graph \
            and self.currentNode == graph.currentNode
    ...
    def initRandomStartNode(self):
        """ Reset graph walk state by set startNode = currentNode to new,
            random possition in graph. 
        """
    ...
    @classmethod
    def getNodeListSum(cls, edgeList):
    ...
    @classmethod
    def getNodeByRangeHit(cls, edgeList,  hit):
        """ Return node in edge such as sum of scores of previous edges 
            is less than hit  and sum + edge score is greater than hit.
    ...
    def next(self,  stopProbability = 0):
        ...
        try:
            ...
            overalSum = self.getNodeListSum(edges)
            if overalSum > 1:
                key = self.randomGenerator.randint(1, overalSum )
            else:
                key = overalSum
    ...
    def getShortestPath (self,  start,  end,  ignoredNodes = None):
        """ Finds shortest path before start and end node, ignoring listed 
            nodes.
...
class TestShortestPath(TestCase):
    ...
    def testPathWithBlockedNodes(self):
        assert self.graph.getShortestPath('1', '6', ['3']) \
            == ['1', '2', '5', '6']
...
class TestBrokenShortests(unittest.TestCase):
    ...
    def testFirstToFirst(self):
        assert self.dependencyGraph.getShortestPath('first', 'first', ['second']) \
            == None
...
class GraphBuilder(object):
    """ Class that defines helper functions to extract transistions between
        methods 
    ...
    USE_FLAT = 0
    """ Constant for definitionType with meaning 'all transistions are possible,
        scores are even' 
    """
    
    USE_LOCAL = 1
    """ Constant for definitionType with meaning 'get edges from method from
        method attribute' 
    """
    
    USE_GLOBAL = 2
    """ Constant for definitionType with meaning 'all transistions 
        (whole graph) is defined in class itself' 
    """
    
    USE_PROB = 3
    """ Constant for definitionType with meaning 'all transistions are possible,
        all edges to given node 
   ...
   """
    ...
    @classmethod
    def generateDependencyGraph(cls,  sourceClass,  methods = None):
        ...
        """
        if not methods:
            methods = getMatchedTypes(sourceClass,   [MethodType])
        
        definitionType = getattr(sourceClass, GraphBuilder.defVarName, 
            GraphBuilder.USE_FLAT)
        startNode = getattr(sourceClass, GraphBuilder.startVarName,  None)
        if methods and startNode not in methods:
            startNode = None
        
        return {
          GraphBuilder.USE_FLAT: cls.generateUsingDefaults,
          GraphBuilder.USE_LOCAL:     cls.generateUsingLocal,
          GraphBuilder.USE_GLOBAL:  cls.generateUsingGlobal,
          GraphBuilder.USE_PROB:       cls.generateUsingProbability,
        }[definitionType](sourceClass = sourceClass,  methods = methods,  startNode = startNode)    
    
    @classmethod    
    def generateUsingDefaults(cls,  sourceClass,  methods,  startNode = None):
        """ Generate dependency graph with flat probabilities 
            (all edges are even, from every node goes edges to all other
    ...
    @classmethod
    def generateUsingLocal(cls,  sourceClass,  methods,  startNode = None):
        ...
        for method in methods:
            obj = getattr(sourceClass,  method)
    ...
    @classmethod
    def generateUsingGlobal(cls,  sourceClass,  methods = None,  startNode = None):
        """ Generate dependency graph from graph defined in class.
            Transistions from and to methods not listed will be discarded.
            
            .. See generateDependencyGraph
        """
        graph = getattr(sourceClass, GraphBuilder.graphVarName,  None)
        if graph is None:
            raise DependencyDeffinitionError(
                "no graph defined while using global graph dependencies")
        else:
            graph = copy(graph)
    ...
    @classmethod
    def generateUsingProbability(cls,  sourceClass,  methods, startNode = None, 
...
class TestGraphGenerator(TestCase):
    ...
    def buildGraphsAndCompare(self,  reference,  buildMethod,  methods):
        referenceGraph = DependencyGraph(graph = reference,  
            startNode = self.GraphClass.startingPoint)
        self.GraphClass.definitionType = buildMethod
        generatedGraph = GraphBuilder.generateDependencyGraph(self.GraphClass,
            methods)
    ...
    nonUniformMethods = ['startNode', 'secondNode', 'switchNode', 'lastNode', 
        'unreachableNode']
    ...
    def testGlobal(self):
        assert self.buildGraphsAndCompare(reference = self.nonUniformGraph,
            buildMethod = GraphBuilder.USE_GLOBAL,
            methods = self.nonUniformMethods)
    ...
    nonUniformMethodsShort = ['startNode', 'secondNode', 'switchNode',
        'unreachableNode']
    
    def testBlockedMethodGlobal(self):
        assert self.buildGraphsAndCompare(self.nonUniformGraphShort,
            GraphBuilder.USE_GLOBAL, self.nonUniformMethodsShort)
    
    def testBlockedMethodLocal(self):
        assert self.buildGraphsAndCompare(self.nonUniformGraphShort,
            GraphBuilder.USE_LOCAL, self.nonUniformMethodsShort)
    ...
    def testProbability(self):
        assert self.buildGraphsAndCompare(self.uniformProbGraph,
            GraphBuilder.USE_PROB, self.uniformMethods)
    
    uniformFlatGraph =  {
        'startNode':[('startNode', 1), ('lastNode', 1),
            ('unreachableNode', 1)], 
        'lastNode':[('startNode', 1), ('lastNode', 1), ('unreachableNode', 1)], 
        'unreachableNode':[('startNode', 1), ('lastNode', 1), 
            ('unreachableNode', 1)], 
                            }
    
    def testFlat(self):
        assert self.buildGraphsAndCompare(self.uniformFlatGraph,
            GraphBuilder.USE_FLAT, self.uniformMethods)
