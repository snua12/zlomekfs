...
{
	syp_error ret =
		dbus_release_syplog_name(connection, err_struct, &syplogger);
	if (ret == NOERR)
		return TRUE;
	else
		return FALSE;
...
static message_handle_state_e dbus_handle_log_message(DBusConnection *
													  connection,
													  DBusError * err_struct,
													  DBusMessage * msg)
{
	syp_error ret =
		dbus_handle_syplog_message(connection, err_struct, msg, &syplogger);
	switch (ret)
	{
	case NOERR:
...
void stop_dbus_control(void)
{
	...
	else
	{
		...
		if (dbus_provider_add_listener(&dbus_provider, dbus_add_zfsd_name,
									   dbus_release_zfsd_name,
									   dbus_handle_zfsd_message) != TRUE)
			message(LOG_WARNING, FACILITY_DBUS | FACILITY_ZFSD,
					"Can't add dbus zfsd state provider\n");


		if (dbus_provider_add_listener(&dbus_provider, dbus_add_log_name,
									   dbus_release_log_name,
									   dbus_handle_log_message) != TRUE)
			message(LOG_WARNING, FACILITY_DBUS | FACILITY_ZFSD,
