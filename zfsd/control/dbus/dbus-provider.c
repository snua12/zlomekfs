...
int dbus_provider_start(dbus_state_holder settings_struct,
						DBusBusType bus_type)
{
	...
	settings_struct->connection =
		dbus_bus_get(bus_type, &(settings_struct->error));
		 ...
		 listener_index++)
	{
		if (settings_struct->listeners[listener_index].add_name
			(settings_struct->connection, &(settings_struct->error)) != TRUE)
...
int dbus_provider_end(dbus_state_holder settings_struct)
{
		 ...
		 listener_index++)
	{
		if (settings_struct->listeners[listener_index].release_name
			(settings_struct->connection, &(settings_struct->error)) != TRUE)
			message(LOG_INFO, FACILITY_DBUS,
					"Can't release name for listener");
	}
							   ...
							   dbus_message_handler_t handle_message)
{
	...
	dbus_listener current =
		&(settings_struct->listeners[settings_struct->listener_count++]);
