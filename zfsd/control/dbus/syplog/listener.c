...
static syp_error handle_socket_reset_facility(listener controller)
{
	...
	if (ret_code == NOERR)
		ret_code =
			control_wrap_reset_facility(controller->target, new_facility);
...
static void *socket_listen_loop(void *data)
{
	...
	while (status == NOERR)
	{
		...
		bytes_read =
			recv(controller->socket, &next_message, sizeof(message_type),
				 MSG_PEEK);
 ...
 * @return std errors
 */
static syp_error dbus_reply_to_ping(logger target, DBusMessage * msg,
									DBusConnection * conn)
{
	...
	{
		do_log(target, LOG_WARNING, FACILITY_LOG, "Syplog ping without arg\n");
		param = "";
	}
	...
	do_log(target, LOG_DEBUG, FACILITY_LOG, "ping called with %s\n", param);
...
static syp_error handle_dbus_reset_facility(logger target, DBusMessage * msg)
{
	...
	else if (SYPLOG_FACILITY_DBUS_TYPE !=
							   ...
							   DBusError * err_struct, logger err_target)
{
	...
	dbus_bus_add_match(connection, SYPLOG_SIGNAL_RECEIVE_RULE, err_struct);	// see 
																			// 
	// signals 
	// from 
	// the 
...
syp_error dbus_handle_syplog_message(DBusConnection * conn,
									 DBusError * err_struct UNUSED,
									 DBusMessage * msg, logger target)
{
	if (dbus_message_is_method_call
		(msg, SYPLOG_DBUS_INTERFACE, SYPLOG_MESSAGE_PING_NAME))
		return dbus_reply_to_ping(target, msg, conn);

	if (dbus_message_is_signal
		(msg, SYPLOG_DBUS_INTERFACE, SYPLOG_SIGNAL_SET_LOG_LEVEL_NAME))
		return handle_dbus_set_log_level(target, msg);

	if (dbus_message_is_signal
		(msg, SYPLOG_DBUS_INTERFACE, SYPLOG_SIGNAL_SET_FACILITY_NAME))
		return handle_dbus_set_facility(target, msg);

	if (dbus_message_is_signal
		(msg, SYPLOG_DBUS_INTERFACE, SYPLOG_SIGNAL_RESET_FACILITY_NAME))
...
static void *dbus_listen_loop(void *data)
{
	...
	while (status == NOERR)
	{
		...
		status =
			dbus_handle_syplog_message(controller->dbus_conn,
									   &(controller->dbus_err), msg,
									   controller->target);
							...
							const char *name UNUSED)
{
	...
	controller->dbus_conn =
		dbus_bus_get(DBUS_BUS_SYSTEM, &(controller->dbus_err));
	...
	{
		do_log(target, LOG_ERROR, FACILITY_LOG,
			   "Got NULL connection from dbus\n");
		ret_code = ERR_DBUS;
		goto FINISHING;
	}
	...
	ret_code =
		dbus_add_syplog_name(controller->dbus_conn, &(controller->dbus_err),
							 controller->target);
	if (ret_code != NOERR)
		goto FINISHING;
