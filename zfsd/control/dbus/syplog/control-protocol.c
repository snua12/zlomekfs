...
*/
static syp_error send_message_to(int msg_socket, const void *message,
								 size_t message_len, const struct sockaddr *to,
								 socklen_t tolen)
{
 ...
 * @return ERR_BAD_PARAMS, ERR_TRUNCATED, system related errors
*/
static syp_error receive_message_from(int msg_socket, void *message,
									  ssize_t * message_len,
									  ...
									  socklen_t * fromlen)
{
	...
	bytes_received =
		recvfrom(msg_socket, message, *message_len, 0, from, fromlen);
	if (bytes_received == -1)
	{
		return sys_to_syp_error(errno);
	}
...
*/
static syp_error send_uint32_action_to(int msg_socket, message_type type,
									   uint32_t data,
...
*/
static syp_error receive_uint32_action_from(int msg_socket,
											message_type * type,
											uint32_t * data,
											struct sockaddr *from,
											socklen_t * fromlen)
{
	...
	ret_code =
		receive_message_from(msg_socket, (void *)message, &chars_read, from,
							 fromlen);
	if (ret_code != NOERR)
		return ret_code;
 ...
 */
static syp_error receive_typed_uint32_action_from(int msg_socket,
												  message_type type,
												  uint32_t * data,
												  struct sockaddr *from,
												  socklen_t * fromlen)
{
	...
	ret_code =
		receive_uint32_action_from(msg_socket, &received_type, data, from,
								   fromlen);
	if (ret_code != NOERR)
		return ret_code;
...
{
	return send_uint32_action_to(msg_socket, MESSAGE_SET_LEVEL, level, to,
								 tolen);
}
...
{
	return receive_typed_uint32_action_from(msg_socket, MESSAGE_SET_LEVEL,
											level, from, fromlen);
}
...
syp_error set_facility_sendto(int msg_socket, facility_t facility,
							  const struct sockaddr * to, socklen_t tolen)
{
	return send_uint32_action_to(msg_socket, MESSAGE_SET_FACILITY, facility,
								 to, tolen);
}
...
syp_error set_facility_receive_from(int msg_socket, facility_t * facility,
									...
									socklen_t * fromlen)
{
	return receive_typed_uint32_action_from(msg_socket, MESSAGE_SET_FACILITY,
...
syp_error reset_facility_sendto(int msg_socket, facility_t facility,
								const struct sockaddr * to, socklen_t tolen)
{
	return send_uint32_action_to(msg_socket, MESSAGE_RESET_FACILITY, facility,
								 to, tolen);
}
