...
/* ! \file \brief Network protocol for controlling logger remotely.
   ...
   network specific operations and transformations, doesn't define what to do
   with received data.

   This functions shoud be used in some listen loop, data received may be
   checked to be valid and if logger needs to be locked, caller should ensure
   this.

   On server side (logger listen) *_receive should be called. Caller should
   check the type of first message on network stack beforehand (for example by 
   PEEK the type). On client side (remote control) *_send functions should be
   called. */
