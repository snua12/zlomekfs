...
/* This code implements the MD5 message-digest algorithm. The algorithm is due 
   ...
   The functions MD5HexDigit and MD5HexFinal, the BIG x LITTLE ENDIAN
   detection fixed and other minor changes by Josef Zlomek
   <josef.zlomek@email.cz> in 2001. This code is also in the public domain
   (see upper).

   Conversion to GNU coding style by Josef Zlomek <josef.zlomek@email.cz> in
   2003.  */
...
static void byteReverse(unsigned char *buf, unsigned longs)
{
	...
	do
	{
		t = (uint32_t) ((buf[3] << 24) | (buf[2] << 16) | (buf[1] << 8) |
						buf[0]);
		*(uint32_t *) buf = t;
		buf += 4;
	}
...
void MD5Final(unsigned char digest[MD5_SIZE], MD5Context * ctx)
{
	unsigned count;
	unsigned char *p;
...
void MD5HexFinal(unsigned char digest[MD5_SIZE * 2], MD5Context * ctx)
{
	unsigned count;
	unsigned char *p;
	...
	*p++ = 0x80;
	...
	p = (unsigned char *)ctx->buf;
	for (count = 0; count < 16; count++, p++)
	{
		digest[count * 2] = MD5HexDigit((*p & 0xf0) >> 4);
		digest[count * 2 + 1] = MD5HexDigit(*p & 0xf);
	}
