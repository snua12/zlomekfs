...
static void fuse_kernel_invalidate_inode(struct fuse_chan *ch, fuse_ino_t ino)
{
	message(LOG_INFO, FACILITY_ZFSD,
			"fuse_kernel_invalidate_inode: ino = %d\n", ino);
...
static hash_t inode_map_ino_hash(const void *x)
{
	return ((const struct inode_map *)x)->ino;
}

static hash_t inode_map_fh_hash(const void *x)
{
	return ZFS_FH_HASH(&((const struct inode_map *)x)->fh);
}

static int inode_map_ino_eq(const void *x, const void *y)
{
	return ((const struct inode_map *)x)->ino == *(const fuse_ino_t *)y;
}

static void inode_map_ino_del(void *x)
{
	free((struct inode_map *)x);
}

static int inode_map_fh_eq(const void *x, const void *y)
{
	return ZFS_FH_EQ(((const struct inode_map *)x)->fh, *(const zfs_fh *)y);
}

/* Return 0 if not found */
static fuse_ino_t fh_get_inode(const zfs_fh * fh)
{
...
/* FIXME? This means that (find zfs_root) will permanently pin a lot of
   memory... */
static fuse_ino_t fh_to_inode(const zfs_fh * fh)
{
	void **slot;
	struct inode_map *map;
...
static const zfs_fh *inode_to_fh(fuse_ino_t ino)
{
	struct inode_map *map;
...
static void inode_map_init(void)
{
	zfsd_mutex_init(&inode_map_mutex);
	inode_map_ino =
		htab_create(100, inode_map_ino_hash, inode_map_ino_eq,
					inode_map_ino_del, NULL);
	inode_map_fh =
		htab_create(100, inode_map_fh_hash, inode_map_fh_eq, NULL, NULL);
	next_ino = FUSE_ROOT_ID;
}

static void inode_map_destroy(void)
{
...
static void sattr_from_req(sattr * attr, fuse_req_t req)
{
...
static void zfs_fuse_readlink(fuse_req_t req, fuse_ino_t ino)
{
	const zfs_fh *fh;
	zfs_fh args;
	read_link_res res;
			   ...
			   mode_t mode, dev_t rdev)
{
	...
	message(LOG_INFO, FACILITY_ZFSD,
			"FUSE: mknod parent=%d, name=%s, mode=%o\n", parent, name, mode);
	fh = inode_to_fh(parent);
	if (fh == NULL)
	{
		err = EINVAL;
				 ...
				 const char *name)
{
	...
	message(LOG_INFO, FACILITY_ZFSD,
			"FUSE: symlink dest=%s, parent=%d, name=%s\n", dest, parent, name);
	fh = inode_to_fh(parent);
	if (fh == NULL)
	{
		err = EINVAL;
				...
				fuse_ino_t newparent, const char *newname)
{
	...
	message(LOG_INFO, FACILITY_ZFSD,
			"FUSE: rename parent=%d, name=%s, newparent=%d, newname=%s\n",
			  ...
			  const char *newname)
{
	...
	message(LOG_INFO, FACILITY_ZFSD,
			"FUSE: link ino=%d, parent=%d, newname=%s\n", ino, newparent,
			newname);
	fh = inode_to_fh(ino);
	if (fh == NULL)
	{
			   ...
			   off_t off, struct fuse_file_info *fi)
{
	...
	while (done < size);
	message(LOG_DEBUG, FACILITY_DATA, "replying size %d\n", size);
...
zfs_fuse_release(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi)
{
	...
	/* Sync is not needed - its kernel responsibility to call write for all
	   data (void)fuse_kernel_sync_inode(fuse_ch, ino); */
	cap = (zfs_cap *) (intptr_t) fi->fh;
...
static void free_dir_list_array(dir_list * list)
{
	dir_entry *entries;
	size_t i;
				 ...
				 struct fuse_file_info *fi)
{
	...
	message(LOG_INFO, FACILITY_ZFSD,
			"FUSE: readdir ino=%d, size=%d, off=%d, fh=%d\n", ino, size, off,
			fi->fh);
	cap = (zfs_cap *) (intptr_t) fi->fh;
	args.cap = *cap;
	args.cookie = off;
	...
	for (i = 0; i < list.n; i++)
	{
		...
		/* Ugly.  The ino returned by zfs_readdir () is only a part of the FH,
		   from a different namespace than the kernel ino.  To get the kernel
		   ino, a lookup is necessary to get the full FH. */
...
static void zfs_fuse_statfs(fuse_req_t req, fuse_ino_t ino)
{
	struct statvfs sfs;
				...
				mode_t mode, struct fuse_file_info *fi)
{
	...
	message(LOG_INFO, FACILITY_ZFSD,
			"FUSE: create parent=%d, name=%s, mode=%o\n", parent, name, mode);
	fh = inode_to_fh(parent);
	if (fh == NULL)
	{
		err = EINVAL;
...
static const struct fuse_lowlevel_ops zfs_fuse_ops = {
	...
	/* .access not necessary, the filesystem should be mounted with -o
	   default_permissions */
...
static void *kernel_main(ATTRIBUTE_UNUSED void *data)
{
	...
	while (!thread_pool_terminate_p(&kernel_pool))
	{
		...
		{
			message(LOG_NOTICE,
					FACILITY_ZFSD | FACILITY_NET | FACILITY_THREADING,
					"Kernel thread terminating\n");
			break;
		}
