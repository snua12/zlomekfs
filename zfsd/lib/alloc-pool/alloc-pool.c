...
/* Copyright (C) 1987, 1997, 1998, 1999, 2000, 2001, 2003 Free Software
   Foundation, Inc. Contributed by Daniel Berlin <dan@cgsoftware.com>
...
typedef struct allocation_object_def
{
	...
	union
	{
		...
		/* Because we want any type of data to be well aligned after the ID,
		   the following elements are here.  They are never accessed so the
		   allocated object may be even smaller than this structure.  */
		char *align_p;
		double align_d;
		uint64_t align_i;
...
#ifdef ENABLE_CHECKING
/*! Last used ID.  */
static alloc_pool_id_t last_id;
#endif
...
#ifdef ENABLE_CHECKING
	/* Increase the last used ID and use it for this pool. ID == 0 is used for 
	   free elements of pool so skip it.  */
	last_id++;
	if (last_id == 0)
		last_id++;
...
void *pool_alloc(alloc_pool pool)
{
	...
	message(LOG_DEBUG, FACILITY_DATA, "POOL ALLOC %s %p %p\n", pool->name,
