...
/* Copyright (C) 2010 Rastislav Wartiak
   ...
   zlomekFS is free software; you can redistribute it and/or modify it under
   the terms of the GNU General Public License as published by the Free
   Software Foundation; either version 2, or (at your option) any later
   version.

   zlomekFS is distributed in the hope that it will be useful, but WITHOUT ANY 
...
{
	return (!strcmp
			(((const struct dirhtab_item_def *)x)->name,
...
int32_t
version_readdir_fill_dirhtab(internal_dentry dentry, time_t stamp, long ino,
							 char *name)
{
	...
	i.ino = ino;
	i.name = name;				// only link for now, will copy it later if
	// needed
	i.stamp = stamp;

	zfsd_mutex_lock(&dentry->fh->mutex);
	x = (struct dirhtab_item_def **)htab_find_slot(dentry->dirhtab, &i,
												   INSERT);
	if (!x)
	{
...
bool version_load_interval_tree(internal_fh fh)
{
	...
	else
	{
		...
		{
			if (!interval_tree_read
				(fh->versioned, fd, st.st_size / sizeof(interval)))
			{
...
/*! \brief Parse version suffix (datetime format) Parse time stamp version
   ...
   version suffix \retval ZFS_OK Version suffix was successfully parsed
   \retval ENOENT version suffix is not correct */
int32_t version_get_filename_stamp(char *name, time_t * stamp, int *orgnamelen)
{
	char *x = name;
	...
	if ((x = strchr(name, VERSION_NAME_SPECIFIER_C)))
	{
		...
		else
		{
			...
			*stamp = mktime(&tm);
			...
			{
				message(LOG_WARNING, FACILITY_VERSION,
						"Cannot convert tm struct to time.\n");
				RETURN_INT(ENOENT);
			}
		}
...
/*! \brief Compare Compare two version items. Function is used by Q-sort in
   ...
   version_item_def */
static int comp_version_items(const void *i1, const void *i2)
{
...
/*! \brief Copy data into version file Copy data from current file to version 
   ...
   of data \param newdata Buffer with data to be written (to compare it only, 
   no write) */
int32_t
version_copy_data(int fd, int fdv, uint64_t offset, uint32_t length,
				  data_buffer * newdata)
{
	TRACE("");
	...
	message(LOG_DEBUG, FACILITY_VERSION,
			"version_copy_data: fd=%d, fdv=%d, offset=%lld, length=%d\n", fd,
			fdv, offset, length);
...
read_again:
	...
	{
		message(LOG_WARNING, FACILITY_VERSION,
				"old data read requested: %d, read only: %d\n", to_read, r);
	}
	...
	{
		message(LOG_DEBUG, FACILITY_VERSION,
				"new data same as old data, no version write\n");
		RETURN_INT(ZFS_OK);
	}
...
write_again:
	...
	{
		message(LOG_WARNING, FACILITY_VERSION,
				"new data write requested: %d, written only: %d\n", to_write,
				r);
	}
