...
static hash_t hardlink_list_hash(const void *x)
{
...
static int hardlink_list_eq(const void *x, const void *y)
{
...
hardlink_list hardlink_list_create(unsigned int nelem, pthread_mutex_t * mutex)
{
	hardlink_list hl;
...
void hardlink_list_empty(hardlink_list hl)
{
...
void hardlink_list_destroy(hardlink_list hl)
{
	hardlink_list_entry entry, next;
...
/*! Insert hardlink [PARENT_DEV, PARENT_INO, NAME] to hardlink list HL. If
   COPY is true make a copy of the NAME. Return true if it was really
   inserted.  */
					 ...
					 uint32_t parent_ino, string * name, bool copy)
{
	...
	if (*slot)
	{
		...
		if (!copy)
		{
			/* If we shall not copy NAME the NAME is dynamically allocated and 
			   caller does not free it so we have to free it now.  */
			free(name->str);
		}
		return false;
	}
...
/*! Return true if hardlink [PARENT_DEV, PARENT_INO, NAME] is a member of
   hardlink list HL.  */
...
/*! Delete hardlink list entry ENTRY from hardlink list HL. Return true if it 
   was really deleted.  */

bool hardlink_list_delete_entry(hardlink_list hl, hardlink_list_entry entry)
{
	void **slot;
...
unsigned int hardlink_list_length(hardlink_list hl)
{
	CHECK_MUTEX_LOCKED(hl->mutex);
...
void print_hardlink_list(FILE * f, hardlink_list hl)
{
	hardlink_list_entry entry;
...
void debug_hardlink_list(hardlink_list hl)
{
...
void initialize_hardlink_list_c(void)
{
...
void cleanup_hardlink_list_c(void)
{
	zfsd_mutex_lock(&hardlink_list_mutex);
#ifdef ENABLE_CHECKING
	if (hardlink_list_pool->elts_free < hardlink_list_pool->elts_allocated)
		message(LOG_WARNING, FACILITY_MEMORY,
				"Memory leak (%u elements) in hardlink_list_pool.\n",
				hardlink_list_pool->elts_allocated -
				hardlink_list_pool->elts_free);
#endif
