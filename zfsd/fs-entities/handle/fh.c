...
/*! Lookup the internal dentry by name but do not lock it.
...
/*! Lookup the internal dentry by name and lock it.
    \param vol Volume whose root is returned if parent == NULL.
    \param parent Directory in which the dentry is being looked up.
    \param name Name of the dentry.  */
...
internal_dentry_unlock (volume vol, internal_dentry dentry)
{
  ...
  message (LOG_LOCK, FACILITY_DATA | FACILITY_THREADING, "FH %p UNLOCK, by %lu at %s:%d\n",
...
internal_fh_destroy_stage1 (internal_fh fh)
{
  ...
  message (LOG_DEBUG, FACILITY_DATA, "FH %p DESTROY, by %lu\n", (void *) fh,
           (unsigned long) pthread_self ());
...
internal_fh_destroy_stage2 (internal_fh fh)
{
  ...
  message (LOG_DEBUG, FACILITY_DATA, "FH %p DESTROYED, by %lu\n", (void *) fh,
           (unsigned long) pthread_self ());
  ...
  if (is_owned (dentry))
    {
      ...
      message (LOG_DEBUG, FACILITY_DATA, "FH %p DELETE, by %lu\n", (void *) dentry->fh,
               (unsigned long) pthread_self ());
...
cleanup_fh_c (void)
{
  ...
  if (fh_pool->elts_free < fh_pool->elts_allocated)
    message (LOG_WARNING, FACILITY_MEMORY, "Memory leak (%u elements) in fh_pool.\n",
             fh_pool->elts_allocated - fh_pool->elts_free);
  if (dentry_pool->elts_free < dentry_pool->elts_allocated)
    message (LOG_WARNING, FACILITY_MEMORY, "Memory leak (%u elements) in dentry_pool.\n",
             dentry_pool->elts_allocated - dentry_pool->elts_free);
  if (vd_pool->elts_free < vd_pool->elts_allocated)
    message (LOG_WARNING, FACILITY_MEMORY, "Memory leak (%u elements) in vd_pool.\n",
