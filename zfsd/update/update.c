...
/*! \brief Queue of file handles for slow updating or reintegrating.
   Protected by #update_slow_queue_mutex. File handles are processed by one
   thread from #update_pool, referenced by #slow_update_worker. */
...
/*! \brief Pointer to thread that is performing slow update. Protected by
   #update_slow_queue_mutex */
static thread *slow_update_worker;

/*! \brief How long at least will the slow update worker sleep after aborted
   by ZFS_SLOW_BUSY */
						...
						varray * blocks)
{
	...
	/* remove blocks modified locally, we don't want to update those, and
	   store result to blocks */
								 ...
								 uint64_t version)
{
	...
	/* if there is an updated tree, clear it, add contects of modified tree
...
/*! \brief Clear the tree of updated intervals and set version of file.
   Wrapper for #update_file_clear_updated_tree_1.  \param fh File handle of
   the file.  \param version New version of the file. */
static int32_t update_file_clear_updated_tree(zfs_fh * fh, uint64_t version)
{
	volume vol;
					...
					uint64_t size)
{
	...
	/* we want to change only size, -1 for other structure members mean no
	   changing */
...
/*! \brief Update parts of file from remote file. The core function for
   updating file contents from remote file. Used either for updating part of
   file that user requested, or for all blocks not updated yet, via background 
   update thread. Each block is first checked if it's really different from
   remote file, by md5 hash comparing. May be called from slow update worker
   thread, it will check for slow connections usage before time consuming
   remote functions, and eventually abort updating. \param cap Capability of
   file to be updated. \param blocks List of blocks to update. \param args
   List of blocks for md5 comparing. \param idx Number of block to start
   searching from. \param slow Determines if it should check for requests
   pending on slow lines and abort if there are some. */
					 ...
					 unsigned int *idx, bool slow)
{
	...
	args->cap = *cap;
	message(LOG_LOOPS, FACILITY_DATA | FACILITY_NET,
			"update_file_blocks_1(): before remote_md5sum\n"
			"md5 args count %u", args->count);
#ifdef DEBUG
	for (i = 0; i < args->count; i++)
	{
		message(LOG_FUNC, FACILITY_DATA,
				"md5 args %u: offset %llu, length %u\n", i, args->offset[i],
				args->length[i]);
	}
#endif
	...
	message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
			"update_file_blocks_1(): after remote_md5sum, result %d count %d\n",
			r, remote_md5.count);
	if (r != ZFS_OK)
		RETURN_INT(r);

	/* no sums got calculated, intervals requested probably doesn't exist
	   remotely (file got truncated) */
	...
	message(LOG_FUNC, FACILITY_DATA | FACILITY_NET,
			"update_file_blocks_1(): before local_md5sum\n");
	r = local_md5sum(&local_md5, args);
	message(LOG_FUNC, FACILITY_DATA | FACILITY_NET,
			"update_file_blocks_1(): after local_md5sum\n");
	if (r != ZFS_OK)
		RETURN_INT(r);
	...
	message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
			"update_file_blocks_1(): version local: %llu,"
			"master: %llu, md5: %llu\n", dentry->fh->attr.version,
			dentry->fh->meta.master_version, remote_md5.version);

	/* check if there file version on master node changed from what we assumed 
	   in our metadata */
	if (dentry->fh->attr.version == dentry->fh->meta.master_version
		&& dentry->fh->meta.master_version != remote_md5.version)
	{
		/* in that case, the whole file should be reupdated */
		message(LOG_DEBUG, FACILITY_DATA | FACILITY_NET,
				"update_file_blocks_1(): oops\n");
		release_dentry(dentry);
		zfsd_mutex_unlock(&vol->mutex);
		zfsd_mutex_unlock(&fh_mutex);
	...
	/* If the size of remote file differs from the size of local file truncate 
	   local file.  */
	...
	/* Delete the same blocks from MODIFIED interval tree and add them to
	   UPDATED interval tree (overwrite what was marked as modified). (I think 
	   this shouldn't happen during normal update, probably only during
	   conflict resolution?) */
	...
	for (i = 0; i < local_md5.count; i++)
	{
		...
		if (local_md5.length[i] == remote_md5.length[i]
			&& memcmp(local_md5.md5sum[i], remote_md5.md5sum[i],
					  MD5_SIZE) == 0)
		{
	...
	/* Process all blocks, update those with different local and remote
	   checksums. */
	for (i = 0, j = *idx; i < remote_md5.count; i++)
	{
		...
		if (i >= local_md5.count	// remote file was bigger
			|| local_md5.length[i] != remote_md5.length[i]	// remote file was 
															// 
			// bigger
			|| memcmp(local_md5.md5sum[i], remote_md5.md5sum[i], MD5_SIZE) != 0)	// md5 
																					// 
			// hashes 
			// not 
			// equal
		{
			...
			if (slow)
			{
				...
				{
					message(LOG_NOTICE,
							FACILITY_THREADING | FACILITY_DATA | FACILITY_NET,
							"Slow connections busy, aborting update\n");
			...
			else
			{
				...
				/* Copy the part which was not written from local file because 
				   local file was truncated meanwhile.  */
							...
							< remote_md5.offset[i])); j++)
				{
					...
					memcpy(buf2 + start - remote_md5.offset[i],
						   buf + start - remote_md5.offset[i], end - start);
				}
...
/*! \brief Update blocks of local file according to remote file.  Prepares
   the md5sum arguments for #update_file_blocks_1 and calls it.  \param cap
   Capability of the local file.  \param blocks Blocks to be updated.  \param
   modified Flag saying the local file has been modified.  \param slow Just
   passed to #update_file_blocks_1 */
int32_t
...
/*! \brief Reintegrate modified blocks of local file CAP to remote file.
   Function for performing the actual reintegration work.  \param cap
   Capability of the file.  \param slow Determines a slow reintegration,
   checks for slow connections usage and aborts if there are other pending
   requests. */
static int32_t reintegrate_file_blocks(zfs_cap * cap, bool slow)
{
	fattr remote_attr;
	volume vol;
	internal_cap icap;
	...
	for (offset = 0; offset < dentry->fh->attr.size;)
	{
		...
		/* Get offset and number of bytes to reintegrate, the maximum is
		   ZFS_MAXDATA */
		tnode = interval_tree_lookup(dentry->fh->modified, offset);
		if (!tnode)				// nothing more to reintegrate
			break;
		if (INTERVAL_START(tnode) > offset)	// position the offset to the
			// reitegration interval start
		...
		message(LOG_INFO, FACILITY_DATA | FACILITY_NET,
				"Will reintegrate %u bytes starting at offset %u\n",
		...
		message(LOG_DEBUG, FACILITY_DATA, "Read local file OK\n");
		...
		if (count > 0)
		{
			...
			if (slow)
			{
				...
				if (pending_slow_reqs_count > 0)
				{
					message(LOG_NOTICE,
							FACILITY_THREADING | FACILITY_DATA | FACILITY_NET,
							"Slow connections busy, aborting slow reintegration\n");
					zfsd_mutex_unlock(&pending_slow_reqs_mutex);
			...
			if (r != ZFS_OK)
			{
				message(LOG_WARNING, FACILITY_DATA | FACILITY_NET,
						"Write to master failed, aborting\n");
				break;
			}
		}
		...
		{
			message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
					"Deleting reintegrated interval from interval tree\n");
	...
	if (r3 == ZFS_OK)
	{
		...
		{
			message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
					"Changing remote file size from %u to %u\n",
					(unsigned)remote_attr.size,
					(unsigned)dentry->fh->attr.size);
			sattr sa;
	...
	message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
			"Updating versions... diff = %u ...", (unsigned)diff);

	if (diff > 0 && !interval_tree_min(dentry->fh->modified))
	{
		message(LOG_DATA, FACILITY_DATA, "Yes\n");
		if (remote_reintegrate_ver(dentry, diff, NULL, vol) == ZFS_OK)
			version_increase += diff;
	}
	else
	{
		message(LOG_DATA, FACILITY_DATA, "No\n");
	...
	if (dentry->fh->modified->deleted)
	{
		...
		{
			message(LOG_DATA, FACILITY_DATA | FACILITY_NET, "Flushing OK\n");
		}
	}
...
/*! \brief Determine if and how the local file should be updated.  Get the
   attributes from remote file and compare them with attributes of local
   dentry, return what should be updated.  \param[out] attr For returning the
   determined remote attributes.  \retval Bitwise-or combination of #IFH_UPDATE 
   for file/dir contents update, #IFH_REINTEGRATE for reintegration and
   #IFH_METADATA for metadata (mode, uid, gid), including file size and master
   version for regular files.  \see UPDATE_P \see REINTEGRATE_P \see
   METADATA_CHANGE_P */
static int
		 ...
		 bool fh_mutex_locked)
{
	...
	message(LOG_FUNC, FACILITY_DATA | FACILITY_NET, "update_p() enter\n");
	...
	message(LOG_DEBUG, FACILITY_DATA | FACILITY_NET,
			"update_p() got master version %llu, local meta: %llu for %s\n",
...
/*! \brief Fully update regular file with file handle FH.  The main file
   updating function of #update_worker. Determines what should be updated and
   performs it.  Handles the connection status change of volume master of the
   file.  Reschedules the file for further updating if it couldn't finish it.
   \param fh File handle (taken from #update_queue or #update_slow_queue)
   \param slowthread Determines if the thread is slow updater. If the file is
   on volume with different speed, it's resolved. */
static int32_t update_file(zfs_fh * fh, bool slowthread)
{
	varray blocks;
	volume vol;
	internal_dentry dentry;
	...
	case CONNECTION_SPEED_SLOW:
		...
		if (slowthread == false)
		{
			/* The file is on slow connected volume and this thread is not
			   slow updater */
			zfsd_mutex_lock(&update_slow_queue_mutex);
			if (slow_update_worker == NULL)
			{
				/* There is no slow updater running. Make this thread the slow 
				   updater and continue updating */
				message(LOG_DEBUG, FACILITY_NET,
						"Changing updater thread to slow updater\n");
				slow_update_worker = pthread_getspecific(thread_data_key);
				slow_update_worker->u.update.slow = true;
			}
			else
			{
				/* There is slow updater running. Mark file for rescheduling
				   and goto end */
				message(LOG_DEBUG, FACILITY_DATA | FACILITY_NET,
						"Passing file handle to slow update queue\n");
				reschedule_fh = *fh;
	...
	default:
		...
		if (slowthread == true)
		{
			/* The file is on fast connected volume and this thread is slow
			   updater Mark file for rescheduling and end */
			message(LOG_DEBUG, FACILITY_DATA | FACILITY_NET,
					"Passing file handle for fast update queue\n");
			reschedule_fh = *fh;
			goto out;
		}
		break;
	...
	case IFH_REINTEGRATE:
		/* File may change from other node when we are reintegrating file so
		   fall-thru.  */
	...
	if (slow)
	{
		...
		if (pending_slow_reqs_count > 0)
		{
			...
			message(LOG_NOTICE,
					FACILITY_THREADING | FACILITY_DATA | FACILITY_NET,
					"Slow line busy on update_file() start, aborting\n");
			reschedule_fh = *fh;
			r = ZFS_SLOW_BUSY;
			goto out;
		}
	...
	message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
			"update_file() trying to open remote file\n");
	r = cond_remote_open(&cap, icap, &dentry, &vol);
	if (r != ZFS_OK)
	{
		message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
				"update_file() trying to open remote file FAILED\n");
		goto out2;
	}
	message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
			"update_file() trying to open remote file OK\n");
	opened_remote = true;
	...
	if (r == ZFS_OK && (what & IFH_UPDATE))
	{
		/* we are updating */
		message(LOG_FUNC, FACILITY_DATA | FACILITY_NET,
				"update_file() in IFH_UPDATE\n");
		/* change file size according to remote, if needed */
		r = truncate_local_file(&vol, &dentry, fh, attr.size);
		if (r == ZFS_OK)
		{
			message(LOG_DATA, FACILITY_DATA,
					"update_file() truncate_local_file() was OK\n");
			bool modified;
			...
			message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
					"update_file() local version %llu master %llu\n",
					dentry->fh->attr.version, dentry->fh->meta.master_version);
			release_dentry(dentry);
			...
			{
				message(LOG_INFO, FACILITY_DATA | FACILITY_NET,
						"update_file() calling update_file_blocks()\n");
	...
	/* If the file was not completelly updated or reintegrated add it to queue 
	   again.  */
			...
			|| (dentry->fh->meta.flags & METADATA_MODIFIED_TREE) != 0))
	{
		message(LOG_NOTICE, FACILITY_DATA | FACILITY_NET,
				"File not fully updated or reintegrated, rescheduling\n");
  ...
  out2:
	message(LOG_FUNC, FACILITY_DATA | FACILITY_NET, "Entering out2\n");
	r2 = find_capability_nolock(&cap, &icap, &vol, &dentry, NULL, false);
#ifdef ENABLE_CHECKING
	if (r2 != ZFS_OK)
		zfsd_abort();
  ...
  out:
	message(LOG_FUNC, FACILITY_DATA | FACILITY_NET, "Entering out\n");
	if (opened_remote)
	{
		message(LOG_FUNC, FACILITY_THREADING | FACILITY_DATA | FACILITY_NET,
				"cond_remote_close()\n");
		cond_remote_close(&cap, icap, &dentry, &vol);
	}
	if (icap)
	{
		message(LOG_FUNC, FACILITY_DATA | FACILITY_NET, "put_capability\n");
		put_capability(icap, dentry->fh, NULL);
	}
	message(LOG_FUNC, FACILITY_THREADING | FACILITY_DATA | FACILITY_NET,
			"internal_dentry_unlock\n");
	...
	if (!zfs_fh_undefined(reschedule_fh))
	{
		...
		{
			message(LOG_INFO, FACILITY_DATA | FACILITY_NET,
					"Rescheduling file on the update_file() end... to fast queue\n");
		...
		{
			message(LOG_INFO, FACILITY_DATA | FACILITY_NET,
					"Rescheduling file on the update_file() end... to slow queue\n");
			zfsd_mutex_lock(&update_slow_queue_mutex);
...
/*! \brief Update generic file DENTRY with file handle FH on volume VOL if
   needed and wanted.  Uses #update_p() to determine what should be updated
   and performs the intersetion of the result and WHAT via #update().
   \param[in] what What should be updated if needed. Bitwise-or combination of 
					...
					int what)
{
	...
	if ((*volp)->master != this_node)
	{
		...
		message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
				"Update file (if needed):how %d what %d\n", how, what);
		if (how & what)
		{
...
/*! \brief Update generic file DENTRY on volume VOL if needed. DENTRY and
   DENTRY2 are locked before and after this macro. DENTRY2 might be deleted in 
   update.  Do WHAT we are asked to do. */
int32_t
	...
	if ((*volp)->master != this_node)
	{
		...
		if (how & what)
		{
			...
			if (r != ZFS_OK)
			{
				...
				if (fh2->ino != fh->ino)
				{
					r2 = zfs_fh_lookup_nolock(fh2, volp, dentry2p, NULL,
											  false);
					if (r2 == ZFS_OK)
...
/*! \brief Update generic file DENTRY on volume VOL associated with
   capability ICAP if needed. Do WHAT we are asked to do. */
int32_t
					 ...
					 zfs_cap * cap, bool put_cap, int what)
{
	...
	if ((*volp)->master != this_node)
	{
		...
		message(LOG_FUNC, FACILITY_DATA | FACILITY_NET,
				"update_cap_if_needed(): update_p() result: how = %d, what = %d\n",
				how, what);
		if (how & what)
		{
...
/*! \brief Delete file/subtree in place of file DENTRY on volume VOL.  Uses
   #recursive_unlink to delete the desired path.  \param[in] journal_p Add a
   journal entries to appropriate journals. Passed to #recursive_unlink
   \param[in] move_to_shadow_p Passed to #recursive_unlink \param[in]
   destroy_dentry Passed to #recursive_unlink */
int32_t
...
/*! \brief Delete file NAME in directory DIR on volume VOL.  Uses
   #recursive_unlink to delete the desired path.  \param[in] journal_p Add a
   journal entries to appropriate journals. Passed to #recursive_unlink
   \param[in] move_to_shadow_p Passed to #recursive_unlink \param[in]
   destroy_dentry Passed to #recursive_unlink */
...
/*! \brief Check if local and remote files are same.  If the local file NAME
   in directory DIR_FH is the same as remote file REMOTE_FH set SAME to true
   and return ZFS_OK.  Otherwise delete NAME and its subtree from directory
   DIR_FH and set SAME to false.  Use local attributes LOCAL_ATTR and remote
   attributes REMOTE_ATTR for comparing the files. */
static int32_t
...
/*! \brief Synchronize attributes and metadata (including regular file's
   size) of local and remote file.  Synchronize attributes of local file with
   provided attributes of remote file.  The attributes synchronized are:
   modetype, uid, gid, size (for regular files).  \param[in] volp Volume of the 
   local file.  \param[in] dentryp Internal dentry of the local file.
   \param[in] fh ZFS file handle of the local file \param[in] attr Attributes
   of the remote file.  \param[in] local_changed The local attributes got
   changed.  \param[in] remote_changed The remote attributes got changed. */
static int32_t
					   ...
					   bool local_changed, bool remote_changed)
{
	...
	message(LOG_FUNC, FACILITY_DATA | FACILITY_NET,
			"synchronize_attributes(): name=%s, local=%u, remote=%u\n",
			(*dentryp)->fh->meta.name, local_changed, remote_changed);

	if (local_changed && METADATA_ATTR_EQ_P((*dentryp)->fh->attr, *attr))
	{
		/* local attributes were supposed to be changed but actually aren't,
		   just update local metadata then */
	...
	if (local_changed)
	{
		...
		zfsd_mutex_unlock(&fh_mutex);
		message(LOG_FUNC, FACILITY_DATA | FACILITY_NET, "here\n");
		message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
				"attr->version %llu, meta_master version %llu\n",
				attr->version, (*dentryp)->fh->meta.master_version);
...
/*! \brief Create local generic file based on remote attributes Create local
   generic file NAME in directory DIR on volume VOL with remote file REMOTE_FH
   and remote attributes REMOTE_ATTR. DIR_FH is a file handle of the directory. */
static int32_t
				...
				zfs_fh * dir_fh, zfs_fh * remote_fh, fattr * remote_attr)
{
	...
	/* for regular files, create with remote size so it is known before
	   fetching whole content */
...
/*! \brief Create remote generic file based on local attributes Create remote 
   generic file NAME in directory DIR on volume VOL according to local
   attributes ATTR.  DIR_FH is a file handle of the directory.  \param[out]
   res Contains remote file handle and attributes. */
static int32_t
				 ...
				 volume vol, zfs_fh * dir_fh, fattr * attr)
{
	...
	sa.uid = attr->uid;
	sa.gid = attr->gid;
	/* for regular files, create with local file's size so it is known on
	   remote node before reintegrating whole content */
	if (attr->type == FT_REG)
	{
		sa.size = attr->size;
	}
...
/*! \brief Schedule update or reintegration of a not yet enqueued regular
   file.  The scheduling happens only for volumes that are currently connected 
   and if some threads in #update_pool are running.  If the file is on slow
   connected volume and there is a #slow_update_worker thread running, it's
   put into #update_slow_queue. Otherwise, it's put into #update_queue. \param 
   vol Volume the file is on.  \param dentry The dentry of the file. */
static void
...
/*! \brief Lookup the remote file which is in the same place as the local
   file.  \param res Buffer for result of directory operation.  \param fh File 
   handle of the file.  \param dentryp Dentry of the file.  \param volp Volume 
   which the file is on. */
static int32_t
...
/*! \brief Synchronize the local file with the remote file.  The function
   synchronizes metadata (attributes and size) if it's needed, creates
   conflict if there is one.  If the master version changed (without creating
   conflict), local metadata is updated and updated tree cleared.  Updating
   and reintegrating is only scheduled, not performed instantly in here.
   \param[in] vol Volume which the file is on.  \param[in] dentry Dentry of
   the file.  \param[in] fh File handle of the file.  \param[in] attr Remote
   attributes.  \param[in] what What should be updated if needed. Bitwise-or
   combination of #IFH_UPDATE for file/dir contents update, #IFH_REINTEGRATE
   for reintegration and #IFH_METADATA for metadata (mode, uid, gid),
   including file size and master version for regular files.  \param
   same_place True if the remote attributes are for the file in the same place 
   as the local file. */
static int32_t
				 ...
				 int what, bool same_place)
{
	...
	message(LOG_FUNC, FACILITY_DATA | FACILITY_NET,
			"synchronize_file(): name=%s, what=%u, same_place=%u\n",
	...
	local_changed = METADATA_ATTR_CHANGE_P(dentry->fh->meta, dentry->fh->attr)
		|| (METADATA_SIZE_CHANGE_P(dentry->fh->attr, *attr)
			&& (dentry->fh->attr.version > attr->version));
	...
	if (!attr_conflict && data_conflict
		&& (dentry->fh->flags & IFH_REINTEGRATING))
	{
		/* The modify-modify conflict may be caused by reintegration so change 
		   nothing.  */
		release_dentry(dentry);
		zfsd_mutex_unlock(&vol->mutex);
		zfsd_mutex_unlock(&fh_mutex);
	...
	else
	{
		...
		message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
				"synchronize_file() end, versions: local: %llu, master: %llu, meta master: %llu\n",
				dentry->fh->attr.version, attr->version,
				dentry->fh->meta.master_version);

		if (dentry->fh->attr.type == FT_REG)
		{
			/* for the regular files, check if master version changed from
			   what we knew in local metadata */
			if (attr->version > dentry->fh->meta.master_version)
			{
				message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
						"synchronize_file(): master version changed, clearing updated tree\n");
				// dentry->fh->attr.version = dentry->fh->meta.master_version;
				/* if yes, this will update the version and remove updated
				   tree if any */
...
/*! \brief Discard changes to local file LOCAL which is in conflict with
   REMOTE on volume VOL. CONFLICT_FH is a file handle of the cnflict directory 
...
/*! \brief Discard changes to remote file REMOTE which is in conflict with
   LOCAL on volume VOL. CONFLICT_FH is a file handle of the cnflict directory
   containing these two files. */
int32_t
...
/*! \brief Resolve conflict by deleting local file. Resolve conflict by
   deleting local file NAME with local file handle LOCAL_FH and remote file
   handle REMOTE_FH in directory DIR with file handle DIR_FH on volume VOL.
   Store the info about deleted file into RES. */
int32_t
							  ...
							  volume vol)
{
	...
	{
		message(LOG_ERROR, FACILITY_DATA | FACILITY_NET,
...
/*! \brief Resolve conflict by deleting remote file NAME with file handle
   REMOTE_FH in directory DIR on volume VOL. */
int32_t
...
/*! \brief Update the directory DIR on volume VOL with file handle FH, set
...
update_dir(volume vol, internal_dentry dir, zfs_fh * fh, fattr * attr)
{
	...
	{
		/* This happens when we have reintegrated a directory and no other
		   node has changed the directory.  */
		release_dentry(dir);
		zfsd_mutex_unlock(&vol->mutex);
		zfsd_mutex_unlock(&fh_mutex);
		RETURN_INT(ZFS_OK);
	...
	HTAB_FOR_EACH_SLOT(local_entries.htab, slot)
	{
		...
		if (slot2)
		{
			...
			if (ZFS_FH_EQ(meta.master_fh, remote_res.file))
			{
				...
				if (!same)
				{
					/* If the special files have the same file handle but do
					   not have same contents delete the local file because
					   this could have happened if master has deleted metadata 
					   and the new file got the same file handle.  */
			...
			else
			{
				if (local_res.attr.version == meta.master_version)
				{
					/* Local file was not modified and remote file has
					   different file handle, i.e. remote file was deleted and 
					   then created/linked/moved. Because local file was not
					   modified, we can delete it too.  */
...
/*! \brief Reintegrate journal for directory DIR on volume VOL with file
   handle FH. Update version of remote directrory in ATTR. */
static int32_t
		...
		case JOURNAL_OPERATION_ADD:
			...
			else if (r == ENOENT || r == ESTALE)
			{
				...
				else
				{
					...
					{
						/* The file does not exists on master. This can happen 
						   when we linked/renamed a file while master has
						   deleted it. In this situation, delete the local
						   file.  */
			...
			if (r == ZFS_OK)
			{
				if (ZFS_FH_EQ(res.file, entry->master_fh))
				{
					if (!local_exists
						...
						&& entry->master_version != res.attr.version)
					{
						/* File does not exist on local and was modified on
						   master.  Create a delete-modify conflict.  */
...
/*! \brief Reintegrate or update generic file Reintegrate or update generic
   file DENTRY on volume VOL with file handle FH and remote file attributes
   ATTR.  \param[in] how What should be updated if needed. Bitwise-or
   combination of #IFH_UPDATE for file/dir contents update, #IFH_REINTEGRATE
   for reintegration and #IFH_METADATA for metadata (mode, uid, gid),
...
static void update_worker_init(thread * t)
{
	t->dc_call = dc_create();
}

/*! \brief Cleanup update thread DATA. */
static void update_worker_cleanup(void *data)
{
...
/*! \brief The main function of an update thread. Normal update threads get
   their file handles passed from thread performing #update_main(), which also
   regulates them and lets them run by raising their semaphore. With the file
   handle got, they perform update. When a thread becomes slow_updater, it's
   the only one doing that so it can get file handles from the
   #update_slow_queue itself. It's no longer regulated by update_pool because
   it appears as busy to it all the time. When the #update_slow_queue becames
   empty, the slow updater converts back to normal updater and goes idle. */
static void *update_worker(void *data)
{
	thread *t = (thread *) data;
	...
	message(LOG_NOTICE, FACILITY_NET | FACILITY_THREADING,
			"Starting worker update thread...\n");
	...
	while (1)
	{
		...
		message(LOG_INFO, FACILITY_NET | FACILITY_THREADING,
				"Worker update thread: Waking up...\n");
                ...
                if ((t->u.update.slow == true) && (r == ZFS_SLOW_BUSY))
		{
			message(LOG_INFO,
					...
					"update_file() returned ZFS_SLOW_BUSY for slow updater worker, sleeping 5+ seconds\n");
			struct timeval now;
			...
			while (r != ETIMEDOUT)
			{
				message(LOG_DEBUG,
						FACILITY_DATA | FACILITY_NET | FACILITY_THREADING,
						"Worker update thread: waiting for slow reqs count == 0\n");
				while (pending_slow_reqs_count != 0)
				{
					pthread_cond_wait(&pending_slow_reqs_cond,
				...
				message(LOG_DEBUG,
						FACILITY_DATA | FACILITY_NET | FACILITY_THREADING,
						"Worker update thread: waiting for 5 seconds of no activity\n");
				r = pthread_cond_timedwait(&pending_slow_reqs_cond,
										   &pending_slow_reqs_mutex, &timeout);
			}
			zfsd_mutex_unlock(&pending_slow_reqs_mutex);
		}

		/* Put self to the idle queue if not requested to die meanwhile.  */
		message(LOG_INFO, FACILITY_DATA | FACILITY_NET | FACILITY_THREADING,
				"Worker update thread: work done...\n");
		...
		if (t->u.update.slow == true)
		{
			...
			bool succeeded;
			message(LOG_INFO, FACILITY_DATA | FACILITY_NET,
					"Worker slow update thread: check slow update queue...\n");
			zfsd_mutex_lock(&update_slow_queue_mutex);
			/* check if slow_queue is empty */
			if (update_slow_queue.nelem == 0)
			{
				/* slow_queue empty, convert this slow updater thread to
				   normal (idle) updater */
				message(LOG_DEBUG, FACILITY_DATA | FACILITY_NET,
						"empty. Changing to normal updater\n");
			...
			else
			{
				...
				message(LOG_DEBUG, FACILITY_DATA | FACILITY_NET,
						"not empty. get file handle...\n");
				succeeded = queue_get(&update_slow_queue, &t->u.update.fh);
				zfsd_mutex_unlock(&update_slow_queue_mutex);
				if (!succeeded)
				{
					message(LOG_NOTICE, FACILITY_DATA | FACILITY_NET,
							"Worker slow update thread: get file handle...failed\n");
					break;
				}
				message(LOG_DEBUG, FACILITY_DATA | FACILITY_NET,
						"Worker slow update thread: get file handle...succeeded\n");
			}
		}
		...
		if (get_thread_state(t) == THREAD_BUSY)
		{
			if (t->u.update.slow == false)
			{
				...
				message(LOG_INFO, FACILITY_NET | FACILITY_THREADING,
						"Update worker: going idle\n");
				queue_put(&update_pool.idle, &t->index);
				set_thread_state(t, THREAD_IDLE);
			}
			else
			{
				/* slow updater, has file handle to update, wasn't killed,
				   just up the semaphore so it doesn't deadlock in the next
				   while cycle */
	...
	message(LOG_INFO, FACILITY_NET | FACILITY_THREADING,
			"Terminating worker update thread...\n");

	return NULL;
}

/*! \brief Main function of the main update thread. This is the main thread
   in #update_pool. It regulates number of threads there, gets file handles
   from #update_queue, passes them into one idle thread's data and wakes up
   that thread via raising its semaphore. */
static void *update_main(ATTRIBUTE_UNUSED void *data)
{
	zfs_fh fh;
	size_t idx;
	...
	while (!thread_pool_terminate_p(&update_pool))
	{
		...
		/* Get the file handle.  */
		message(LOG_DEBUG, FACILITY_DATA | FACILITY_NET,
				"Main update thread: get file handle...\n");
		zfsd_mutex_lock(&update_queue_mutex);
		...
		{
			message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
					"Main update thread: get file handle...failed\n");
			break;
		}
		message(LOG_DATA, FACILITY_DATA | FACILITY_NET,
				"Main update thread: get file handle...succeeded\n");
		...
		/* Let the thread run.  */
		message(LOG_DEBUG, FACILITY_NET | FACILITY_THREADING,
				"Main update thread: starting worker thread\n");
	...
	message(LOG_NOTICE, FACILITY_NET | FACILITY_THREADING,
			"Terminating main update thread...\n");

	return NULL;
}

/*! \brief Initialize the mutexes and queues for updating, and create the
   #update_pool. */
...
/*! \brief Destroy #update_pool and cleanup the mutexes and queues for
   updating. */
