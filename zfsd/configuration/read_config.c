...
bool read_config_file(const char *file)
{
	...
	while (!feof(f))
	{
		...
		if (*key)				/* There was a configuration directive on the
								   line.  */
		{
			if (value_len)
			{
...
bool read_local_volume_info(string * path, bool reread)
{
	...
	while (!feof(f))
	{
		...
		/* 0 ... ID 1 ... local path 2 ... size limit */
		...
		// TODO: write volume id validator
		...
		// TODO: fix directory separator for windows
