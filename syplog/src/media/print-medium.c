/* ! \file \brief Print medium implementation.
...
/* ! Parse print medium specific params @param argc argv count @param argv
   ignored now @param settings medium structure where to apply given settings
   (non NULL) @return ERR_BAD_PARAMS on wrong argv or settings, NOERR
   otherwise */
static syp_error print_medium_parse_params(int argc, const char **argv,
										   medium settings CHECKING_UNUSED)
{
	int opt;
...
syp_error open_print_medium(medium target, int argc, const char **argv)
{
	...
	if (argv != NULL)
		ret_code =
			print_medium_parse_params(argc, (const char **)argv, target);
	if (ret_code != NOERR)
	{
		goto FINISHING;
	}
