...
/* ! \file \brief Facilities and associated helper functions.

   Facility is a part (or concern) of application which needs to be
   distinguished in logging.

   Current model of facilities is a bitmap. When the bit for particular
   facility is set in bitmap, it means that messages from this facility have
   to be logged.

   Message can be associated to more than one facility. Then it is logged if
   at least one of them is set to be logged.
...
#define	FACILITY_ALL_NAME		"ALL"
	...
	/* ! Translates singular facility to name @param facility singular
	   facility. Multiple ORed facilities leads to unknown / nontrivial
	   response @return user readable string representing given facility or
	   NULL */
	...
	// TODO: multifacilities
	/* ! Translates singular facility name to bit representation @param
	   facility_name singular facility name @return bit representation fo
	   facility or FACILITY_UNKNOWN */
	static inline facility_t facility_from_string(const char *facility_name)
	{
		if (strncmp(facility_name, FACILITY_LOG_NAME, FACILITY_STRING_LEN) ==
			0)
			return FACILITY_LOG;
		if (strncmp
			(facility_name, FACILITY_THREADING_NAME, FACILITY_STRING_LEN) == 0)
			return FACILITY_THREADING;
		if (strncmp(facility_name, FACILITY_NET_NAME, FACILITY_STRING_LEN) ==
			0)
			return FACILITY_NET;
		if (strncmp(facility_name, FACILITY_CACHE_NAME, FACILITY_STRING_LEN) ==
			0)
			return FACILITY_CACHE;
		if (strncmp(facility_name, FACILITY_DATA_NAME, FACILITY_STRING_LEN) ==
			0)
			return FACILITY_DATA;
		if (strncmp(facility_name, FACILITY_MEMORY_NAME, FACILITY_STRING_LEN)
			== 0)
			return FACILITY_MEMORY;
		if (strncmp(facility_name, FACILITY_CONFIG_NAME, FACILITY_STRING_LEN)
			== 0)
			return FACILITY_CONFIG;
		if (strncmp(facility_name, FACILITY_DBUS_NAME, FACILITY_STRING_LEN) ==
			0)
			return FACILITY_DBUS;
		if (strncmp(facility_name, FACILITY_ZFSD_NAME, FACILITY_STRING_LEN) ==
			0)
			return FACILITY_ZFSD;
		if (strncmp(facility_name, FACILITY_VERSION_NAME, FACILITY_STRING_LEN)
			== 0)
			return FACILITY_VERSION;

		if (strncmp(facility_name, FACILITY_NOTHING_NAME, FACILITY_STRING_LEN)
			== 0)
			return FACILITY_NOTHING;
		if (strncmp(facility_name, FACILITY_ALL_NAME, FACILITY_STRING_LEN) ==
			0)
			return FACILITY_ALL;
	...
	static inline facility_t facility_add(facility_t bitmap,
										  facility_t facility)
	{
	...
	static inline facility_t facility_del(facility_t bitmap,
										  facility_t facility)
	{
		return bitmap & ~facility;
	}
	...
	static inline bool_t facility_get_state(facility_t bitmap,
											facility_t facility)
	{
