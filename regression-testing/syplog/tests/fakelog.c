/* ! \file \brief Read messages from console and logs them into /dev/tty \ on
   all levels. Used for testing, if control messages arrives. */
...
int main(int argc UNUSED, char **argv UNUSED)
{
	...
	ret_value =
		open_log(&output_printer, "fakelog", log_settings_count,
				 (const char **)log_settings);
	if (ret_value != NOERR)
	{
		printf("init fatal opening output: %d, %s\n", ret_value,
			   syp_error_to_string(ret_value));
		exit(ret_value);
	}
