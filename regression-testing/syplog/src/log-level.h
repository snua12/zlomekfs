...
/* ! \file \brief Log levels and associated helper functions.

   Log level is in general verbosity of logger. There are defined multiple
   log levels. They are ordered in ascending order from the most important
   (LOG_EMERG the lowest) to the least important (LOG_LOOPS the greatest).
   Messages with greater log level than actual log level (verbosity) of
   logger won't be logged. */
