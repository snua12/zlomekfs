...
void print_shm_medium_help(int fd, int tabs)
{
	...
	tabize_print(tabs, fd,
				 "shm medium writes log to shared memory segment (reads from).\n");
	tabize_print(tabs, fd, "shm medium options:\n");
	tabs++;

	tabize_print(tabs, fd,
				 "--%s=value, -%c value\tshared memory segment key\n",
...
/* ! Parse shm medium specific params @param argc argv count @param argv std
   "main" format params (--segment-key=1024) (non NULL) @param settings medium 
   structure where to apply given settings (non NULL) @return ERR_BAD_PARAMS
   on wrong argv or settings, NOERR otherwise */
static syp_error shm_medium_parse_params(int argc, const char **argv,
										 medium settings)
{
	...
	while ((opt =
		...
		case PARAM_SHM_KEY_CHAR:	// log file name
			((shm_medium) (settings->type_specific))->segment_key =
				atoi(optarg);
			break;
		case '?':
		default:
