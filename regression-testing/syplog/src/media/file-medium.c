/* ! \file \brief File medium implementation.

   File medium takes logs from file and parses them to structures (or vice
   versa). */
...
/* ! Parse file medium specific params @param argc argv count @param argv std
   "main" format params (--log-file=/var/log/zfsd.log) (non NULL) @param
   settings medium structure where to apply given settings (non NULL) @return
   ERR_BAD_PARAMS on wrong argv or settings, NOERR otherwise */
static syp_error file_medium_parse_params(int argc, const char **argv,
										  medium settings)
{
	int opt;
...
static inline syp_error shift_to_begin_if_needed(medium target)
{
	...
	if (target->length > 0)
		if (target->length - pos <
			...
			if (fseek
				(((file_medium) target->type_specific)->handler, 0,
				 SEEK_SET) == SYS_NOERR)
				target->pos = 0;
...
syp_error open_file_medium(medium target, int argc, const char **argv)
{
	...
	case WRITE_LOG:
		...
		if (new_specific->handler == NULL)
			// heuristic for non-existing file FIXME: find appropriate errno
			// and do this only upon it
...
syp_error file_access(medium target, log_struct log)
{
	...
	else if (chars_accessed == 0)
	{
		if (target->kind == WRITE_LOG)
		{
			// if we have written zero bytes, there should be feof and no
			// space
			if (fseek
				(((file_medium) target->type_specific)->handler, 0,
				 SEEK_SET) == SYS_NOERR)
