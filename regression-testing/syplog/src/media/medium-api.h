...
/* ! \file \brief Api for media access functions.

   Medium is set of functions and settings describing the method how and from
   where to read or write logs;

   Medium state is dependent on type (file, memory, socket reader). Medium
   uses formatters to parse logs
...
/* ! Initializes medium according to parameters. @param argv the same format
   as in "main", but parses only medium specific options @param argc argv item 
   count @param target non NULL pointer to medium structure to initialize
...
/* ! Check if argument is recognized by some medium @param arg command line
...
/* ! Close reader and free internal pointers. @param target non NULL pointer
   to initialized medium @return std errors. When error is returned, structure 
   is in undefined state but most likeli internal pointers are freed */
...
/* ! Access Medium. Depends on kind of access this means read log or write log
   ...
   @param target initialized medium (non NULL) @param log message to fill or
   write (non NULL) @return std errors. On error, message may not be written. */
syp_error access_medium(struct medium_def *target, log_struct log);

/* ! prints media options help to fd. @param fd file descriptor to which to
   write help @param tabs how much tabs prepend to help */
