/* ! \file \brief User readable formatter implementation.  */
...
/* ! Print message to string buffer in format defined for user readable
   ...
   NULL string buffer with length at least buffer_len @param buffer_len
   maximal number of chars printed to buffer @return number of chars printed
   or -syp_error on error */
static int32_t fill_buffer(const log_struct message, char *buffer,
						   int32_t buffer_len)
{
...
/* ! Read message from string buffer in format defined for user readable
   formatter @param message non NULL pointer to log struct @param buffer non
   NULL string buffer containing log message in format defined for user
   readable formatter @return number of chars read or -syp_error on error */
static int32_t read_buffer(log_struct message, const char *buffer)
{
	...
	chars_read = sscanf(buffer, "%s\t%s\t%ld/%s\t%s\t%s\t%s\t%s\t%s\n",
						...
						facility_str,
						log_level_str,
						time_str, timezone_str, message->message);
	if (chars_read <= 0)
		return -ERR_SYSTEM;
...
int32_t user_readable_stream_write(log_struct message UNUSED,
								   int socket UNUSED)
{
