/* ! \file \brief Main generic formatter handling functions implementations.
...
/* ! Translation table between formatter names and pointers to static
   formatter type specific structures. The table must be NULL terminated. One
...
struct formatter_name formatter_translation_table[] = {
	/* 
	   {LOG4J_FORMATTER_NAME, log4j_formatter}, {XML_FORMATTER_NAME,
	   xml_formatter}, */
...
formatter formatter_for_name(const char *formatter_name)
{
	...
	for (table_index = 0;
		 formatter_translation_table[table_index].name != NULL; table_index++)
	{
		if (strncmp
			(formatter_name, formatter_translation_table[table_index].name,
