...
int zfsd_read(data_buffer *buffer, read_args *args)
{
        DC *dc;
        read_res res;
        int error;
        ...
        if (!error) {
                if (!decode_read_res(dc, &res)
                    ...
                    || !finish_decoding(dc))
                        ...
                        if (buffer->user) {
                          if (copy_to_user(buffer->buf.u_wbuf, res.data.buf.k_buf, res.data.len))
                                  error = -EFAULT;
                          else
                                  error = res.data.len;
                        } else {
                          memcpy(buffer->buf.k_buf, res.data.buf.k_buf, res.data.len);
                          error = res.data.len;
                        }
                }
        }
