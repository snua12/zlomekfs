/*! \file
    \brief Main media functions.  

  Implementation of medium type independent functions,
...
/*! Holds translations from stringified names of media operations
    to medium_operation discriminator
*/
struct 
{
...
medium_operation operation_name_to_enum(const char * operation_name)
{
  ...
  for (table_index = 0;
       ...
       table_index ++)
  {
    if (strncmp(operation_name, medium_operation_translation_table[table_index].name,
                OPERATION_NAME_LEN) == 0)
      break;
  }

  // if we end inside of table (not a terminator) we found a operation
  if (table_index >= 0 && medium_operation_translation_table[table_index].name != NULL)
  {
...
/*! Holds translations from stringified names of media
    to medium_type discriminator
*/
struct 
{
...
{
/*
  {NO_MEDIUM_NAME,	NO_MEDIUM},
  {SOCKET_MEDIUM_NAME,	SOCKET_MEDIUM},
*/
...
/*! Translates medium type name to medium_type discriminator.
...
medium_type medium_name_to_enum (const char * medium_type_name) 
{
  ...
  for (table_index = 0;
       medium_translation_table[table_index].name != NULL;
       table_index ++)
  {
    if (strncmp(medium_type_name, medium_translation_table[table_index].name,
                MEDIUM_NAME_LEN) == 0)
      break;
  }
...
void print_media_help (int fd, int tabs)
{
  ...
  tabize_print (tabs, fd, "medium defines where and in what manner logs are written.\n");
