...
user_create (uint32_t id, string *name)
{
  ...
  if (slot1 && slot2 && *slot1 == *slot2)
    {
      ...
      if (!u->marked)
	{
	  message (LOG_WARNING, FACILITY_CONFIG, "Duplicate user ID and name: %" PRIu32 " %s\n",
		   id, name->str);
	  return NULL;
	}
      u->marked = false;
  ...
  if (slot1)
    {
      /* ID is already there.  */
      message (LOG_WARNING, FACILITY_CONFIG, "Duplicate user ID: %" PRIu32 "\n", id);
      return NULL;
    }
  if (slot2)
    {
...
group_create (uint32_t id, string *name)
{
  ...
  if (slot1 && slot2 && *slot1 == *slot2)
    {
      ...
      if (!g->marked)
	{
	  message (LOG_WARNING, FACILITY_CONFIG, "Duplicate group ID and name: %" PRIu32 " %s\n",
		   id, name->str);
	  return NULL;
	}
      g->marked = false;
  ...
  if (slot1)
    {
      /* ID is already there.  */
      message (LOG_WARNING, FACILITY_CONFIG, "Duplicate group ID: %" PRIu32 "\n", id);
      return NULL;
    }
  if (slot2)
    {
